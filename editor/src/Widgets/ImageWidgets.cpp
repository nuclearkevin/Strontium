...
namespace Strontium
{
  TextureWidget::TextureWidget(const std::string &widgetName)
    ...
    , tex3DTex2D(ShaderCache::getShader("tex_3D_tex_2D"))
  ...
  void 
  TextureWidget::texture2DImageLod(Texture2D &texture, const ImVec2& size,
						           const ImVec2 &uv1, const ImVec2 &uv2)
  {
    ...
    ImGui::SliderFloat("LOD", &this->lod, 0.0f, maxLod);

    texture.bind(0);
    this->tex2DLod->addUniformFloat("u_lod", this->lod);
    ...
    this->tex2DLod->launchCompute(iWidth, iHeight, 1);
  ...
  void 
  TextureWidget::texture3DImage(Texture3D& texture, const ImVec2& size, bool lodSlider,
                                const ImVec2& uv1, const ImVec2& uv2)
  {
    // Resize the buffer if necessary.
    if (static_cast<int>(size.x) != this->buffer.getWidth() || 
    ...
    ImGui::Text(this->widgetName.c_str());
    // Array slice slider behavior.
    int maxSlice = static_cast<int>(texture.getDepth()) - 1;
    ImGui::SliderInt("3D Texture Slice", &this->slice, 0, maxSlice);
    ...
    texture.bind(0);
    this->tex3DTex2D->addUniformFloat("u_lod", this->lod);
    this->tex3DTex2D->addUniformFloat("u_slice", (static_cast<float>(this->slice) + 0.5f) / static_cast<float>(maxSlice + 1));
    uint iWidth = static_cast<uint>(glm::ceil(static_cast<float>(this->buffer.getWidth()) / 8.0f));
    uint iHeight = static_cast<uint>(glm::ceil(static_cast<float>(this->buffer.getHeight()) / 8.0f));
    this->buffer.bindAsImage(0, 0, ImageAccessPolicy::Write);
    this->tex3DTex2D->launchCompute(iWidth, iHeight, 1);
    Shader::memoryBarrier(MemoryBarrierType::ShaderImageAccess);
