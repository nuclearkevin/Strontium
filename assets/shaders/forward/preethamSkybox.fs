...
vec3 computeSunLuminance(vec3 e, vec3 s);

void main()
{
  vec3 viewDir = normalize(fragIn.fTexCoords);
  vec3 sunDir = normalize(u_lodDirection.yzw);
  vec3 skyLuminance = 0.05 * calculateSkyLuminanceRGB(sunDir, viewDir, u_turbidity) + computeSunLuminance(viewDir, sunDir);
  fragColour = vec4(skyLuminance, 1.0);
}
...
vec3 computeSunLuminance(vec3 e, vec3 s)
{
  const float sunSolidAngle = PI / 180.0;
